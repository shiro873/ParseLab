export function analyzeText(content: string) {
    ...
    if (!text) return { wordCount: 0, sentenceCount: 0, paragraphCount: 0, longestWord: '', topNWords: [], uniqueWords: [], mostFrequentWord: null };
    ...
    // uniqueWords: distinct words, lowercased, sorted alphabetically
    const uniqueWords = Array.from(freq.keys()).sort();

    // mostFrequentWord: the word with highest frequency, or null if none
