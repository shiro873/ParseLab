...
const jobQueue_1 = require("../src/jobQueue");
...
describe('Metrics', () => {
    ...
    beforeEach(() => {
        ...
        cache = new inMemoryCache_1.InMemoryCache();
        queue = new jobQueue_1.JobQueue({ cache, concurrency: 2 });
        queue.start();
    });
    afterEach(() => {
    ...
    it('should increment processed counter when jobs complete', async () => {
        const initialMetrics = metrics_1.metrics.getMetrics();
        ...
        const finalMetrics = metrics_1.metrics.getMetrics();
    ...
    it('should return metrics with all expected fields', () => {
        const metricsData = metrics_1.metrics.getMetrics();
    ...
    it('should increment retried counter on retries', async () => {
        ...
        const failingQueue = new jobQueue_1.JobQueue({ cache: failingCache, concurrency: 1 });
        failingQueue.start();
        const initialMetrics = metrics_1.metrics.getMetrics();
